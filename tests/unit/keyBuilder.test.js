...
assert('Builds ip key', buildKey('auth-login', 'ip', requestData, 'bf')?.key === 'bf:auth-login:ip:127.0.0.1');
assert(
...
assert('Does not use password key', buildKey('auth-login', 'body.password', requestData, 'bf') === null);
assert(
