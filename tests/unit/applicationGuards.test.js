...
function runAll() {
  ...
  assert(
    'Prototype Pollution detects __proto__ key',
    protoBodyHit?.detector === 'PROTOTYPE_POLLUTION'
  );
  const protoQueryHit = PrototypePollutionDetector.scan({ query: PROTOTYPE_POLLUTION_QUERY });
  assert(
    'Prototype Pollution detects constructor/prototype keys',
    protoQueryHit?.detector === 'PROTOTYPE_POLLUTION'
  );
  const circular = {};
  circular.self = circular;
  assert(
    'Prototype Pollution handles circular objects',
    PrototypePollutionDetector.scan({ body: circular }) === null
  );
  ...
  assert(
    ...
    RequestShapeGuard.scan({ body: { a: 1, b: 2, c: 3, d: 4, e: 5, f: 6 } }, SHAPE_LIMITS)
      ?.pattern === 'maxKeys'
  );
  assert(
    'Request Shape detects excessive array length',
    RequestShapeGuard.scan({ body: { ids: [1, 2, 3, 4] } }, SHAPE_LIMITS)?.pattern ===
      'maxArrayLength'
  );
  assert(
    'Request Shape detects excessive string length',
    RequestShapeGuard.scan({ body: { text: 'x'.repeat(13) } }, SHAPE_LIMITS)?.pattern ===
      'maxStringLength'
  );
  const circularShape = { name: 'safe' };
  circularShape.self = circularShape;
  assert(
    'Request Shape handles circular objects',
    RequestShapeGuard.scan({ body: circularShape }, SHAPE_LIMITS) === null
  );
  assert(
    'Request Shape allows normal data',
    RequestShapeGuard.scan({ body: { tags: ['a', 'b'], name: 'safe' } }, SHAPE_LIMITS) === null
  );
}

test('Application-layer guards', runAll);
