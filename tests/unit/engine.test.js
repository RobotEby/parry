...
assert('Blocked decision exposes reason and status', blocked.reason === 'THREAT' && blocked.statusCode === 400);
assert('Blocked decision exposes detector and severity', blocked.detector === 'SQL_INJECTION' && blocked.severity === 'high');
assert('Blocked decision keeps compatible event shape', blocked.event?.type === 'THREAT' && blocked.event.threats?.[0]?.field === 'body.username');
