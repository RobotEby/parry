...
function analyzeRequest(requestData, context) {
  ...
  const threats = scanTargets(requestData.targets || [], config);
  ...
  if (threats.length > 0) {
    ...
    return {
      ...
      severity: severityForThreats(threats),
      detector: threats[0].detector,
      threats,
      event,
      rateLimit,
      responseExtra: {
...
module.exports = { analyzeRequest, scanTargets };
